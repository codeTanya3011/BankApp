...
class PlanService:
    ...
    @staticmethod
    async def insert_plans_from_excel(
            file_bytes: bytes,
            uow: UnitOfWork
    ) -> None:
        ...
        async with uow:
            for _, row in df.iterrows():
                amount = row["sum"]
                if pd.isna(amount):
                    raise Exception("Сума не може бути пустою")

                period_dt = pd.to_datetime(row["period"], dayfirst=True).date()
                if period_dt.day != 1:
                    raise Exception(f"Невірний період: {period_dt}. Має бути 1-ше число.")
    ...
    async def get_plans_performance(
        ...
        async with uow:
            plans = await uow.plans.get_plans_for_month(
                check_date.year,
                check_date.month
            )
            ...
            for plan in plans:
                ...
                if category_name in ["issuance", "видача"]:
                    _, fact_sum = await uow.plans.get_issued_credits(
                        period_start,
                        check_date
                    )
                elif category_name in ["collection", "percent", "збір", "відсотки"]:
                    _, fact_sum = await uow.plans.get_percent_payments(
                        period_start,
                        check_date
                    )
                else:
                ...
                percent = (
                    round((fact_sum / plan_sum) * 100, 2)
                    if plan_sum > 0 else 0
                )
    ...
    async def year_performance(
            year: int,
            uow: UnitOfWork
    ) -> list[YearPerformanceMonthResponse]:
        ...
        async with uow:
            ...
            for month in range(1, 13):
                ...
                monthly_cache[month] = {
                    "i_count": i_count, "i_sum": i_sum,
                    "p_count": p_count, "p_sum": p_sum
                }
                year_issued_total += i_sum
                year_payments_total += p_sum

            plans = await uow.plans.get_plans_by_year(year)
            plans_map = defaultdict(lambda: {"issuance": 0.0, "collection": 0.0})

            for plan in plans:
                c_name = plan.category.name.lower()
                key = "issuance" if c_name == "видача" else "collection"
                plans_map[plan.period.month][key] = float(plan.sum)

            for month in range(1, 13):
                m = monthly_cache[month]
                i_plan = plans_map[month]["issuance"]
                p_plan = plans_map[month]["collection"]
                ...
                result.append(
                    YearPerformanceMonthResponse(
                        ...
                        issued_fact_sum=m["i_sum"],
                        issued_count=m["i_count"],
                        issued_percent=(round((m["i_sum"] / i_plan) * 100, 2) if i_plan else 0),
                        payments_plan_sum=p_plan,
                        payments_fact_sum=m["p_sum"],
                        payments_count=m["p_count"],
                        payments_percent=(round((m["p_sum"] / p_plan) * 100, 2) if p_plan else 0),
                        issued_part_of_year=(
                            round((m["i_sum"] / year_issued_total) * 100, 2) if year_issued_total else 0),
